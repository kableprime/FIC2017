...
#define H_MAX_BLACK 179
#define S_MIN_BLACK 0
#define S_MAX_BLACK 50
#define V_MIN_BLACK 0
#define V_MAX_BLACK 100
...
int rotire(){
  strcpy(buffer_global, "");
  if(((x_R - x_B) > (x_Y - x_B)) && ((y_R - y_B) > (y_Y - y_B))){
    strcpy(buffer_global, "rs");
    return 1;}
  return 0;
}
int raza_cm=35;
int raza_pixeli=1322;

int miscare(){
	//ma asigur ca urmatoarea mutare nu ma va da jos din ring
	if((x_R < (x_BLACK + raza_pixeli)) && (x_R > (x_BLACK - raza_pixeli)))
		if((y_R < (y_BLACK + raza_pixeli)) && (y_R > (y_BLACK - raza_pixeli)))
			return 1;
	return 0;
}
...
int main(int argc, char* argv[]){
 ...
 // }
   ...
   inRange(HSV, Scalar(H_MIN_BLACK, S_MIN_BLACK, V_MIN_BLACK), Scalar(H_MAX_BLACK, S_MAX_BLACK, V_MAX_BLACK), threshold);
    ...
    y_BLACK=y;
   //identificare galben
   inRange(HSV, Scalar(H_MIN_Y, S_MIN_Y, V_MIN_Y), Scalar(H_MAX_Y, S_MAX_Y, V_MAX_Y), threshold);
		if (useMorphOps)
			morphOps(threshold);
		if (trackObjects)
			trackFilteredObject(x, y, threshold, cameraFeed);
    ...
    y_Y=y;
      ...
      if(rotire() ==0)
			...
			if(miscare() == 1){
        strcpy(buffer, "");
				strcpy(buffer, buffer_global);
        }
			else
				strcpy(buffer, "ssss");
			//sprintf(buffer, buffer_global);
      printf("Valoare trimisa: \n");
			printf("buffer: %s \n", buffer);
